...
const createTaskSchema ={task: z.object({
...
tasks.get('/', async (c) => {
  ...
  const allTasks = userId ? await db.task.findMany({
...
tasks.post('/',
  zValidator('json', z.object({
    body: z.object(createTaskSchema)
  })),
  async (c) => {
    const db = getPrisma(c.env.DATABASE_URL)
  const { body: { task } } = c.req.valid('json');

  const newTask = await db.task.create({
    data: {
      ...task,
      comments: {
        create: task.comment?.map((comment) => ({
...
tasks.put('/',
  zValidator('json', z.object({
    body: z.object(updateTaskSchemea)
  })),
  async (c) => {
    const db = getPrisma(c.env.DATABASE_URL)
  const { body: { task: { comment, ...task } } } = c.req.valid('json');
...
tasks.post('/:id/comments', 
  zValidator('json', z.object({
    body: z.object({comment: createCommentSchema})
  })),
  async (c) => {
  const db = getPrisma(c.env.DATABASE_URL)
  const id = c.req.param('id');
  const { body: { comment } } =  c.req.valid('json')
